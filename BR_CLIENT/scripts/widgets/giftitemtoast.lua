...
local GiftItemToast = Class(Widget, function(self, owner)
    ...
    self.unopened = 0
    self.inst:DoPeriodicTask(1, function() self:Update() end)
...
function GiftItemToast:Update()
    local unopened = #TheInventory:GetUnopenedItems()
    if unopened ~= self.unopened then
        self.unopened = unopened
        self:OnToast(unopened)
		self:EnableClick()
    end
end
...
function GiftItemToast:DoOpenGift()
    if not self.owner:HasTag("busy") then
        ...
        if time - last_click_time > TIMEOUT then
			...
			if self.owner.HUD then
				self.owner.HUD:OpenItemManagerScreen()
			end
        end
    end
end
